...
public struct PersistedConfig: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        firstOutputUID = try c.decodeIfPresent(String.self, forKey: .firstOutputUID)
        channels = try c.decodeIfPresent([String: ChannelSetting].self, forKey: .channels) ?? [:]
        outputVolumes = try c.decodeIfPresent([String: Float].self, forKey: .outputVolumes) ?? [:]
        inputUID = try c.decodeIfPresent(String.self, forKey: .inputUID)
        monitorMicUID = try c.decodeIfPresent(String.self, forKey: .monitorMicUID)
        monitorLastMicUID = try c.decodeIfPresent(String.self, forKey: .monitorLastMicUID)
        monitorChannels = try c.decodeIfPresent([String: [Int]].self, forKey: .monitorChannels) ?? [:]
        showLevels = try c.decodeIfPresent(Bool.self, forKey: .showLevels) ?? true
        monitorEnabled = try c.decodeIfPresent(Bool.self, forKey: .monitorEnabled) ?? false
        monitorOutputUID = try c.decodeIfPresent(String.self, forKey: .monitorOutputUID)
        monitorOutputVolume = try c.decodeIfPresent(Float.self, forKey: .monitorOutputVolume) ?? 0.5
        mainWindowOpen = try c.decodeIfPresent(Bool.self, forKey: .mainWindowOpen) ?? true
    }
}
