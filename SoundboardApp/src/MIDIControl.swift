...
public extension MIDIControl {
    ...
    var midiKey: MIDIKey? {
        guard let deviceID = midiDeviceID, let controlID = midiControlID else { return nil }
        return MIDIKey(deviceID: deviceID, controlID: controlID)
    }
}
...
public protocol MIDIScalar {
    ...
    static func fromSlider(_ value: Double) -> Self // Slider -> value
...
extension Float: MIDIScalar {
    ...
    public static func fromSlider(_ value: Double) -> Float { Float(value) }
...
public final class MIDIButton: MIDIControl, Identifiable {
    ...
    public func setSilently(on newValue: Bool) {
        silent = true
        isOn = newValue
        silent = false
    }
