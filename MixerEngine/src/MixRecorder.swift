...
final class MixRecorder {
    ...
    private func checkFile(_ status: OSStatus) throws {
        if status != noErr { throw RecorderError.file(status) }
    }
    ...
    func close() {
        ...
        if let ref = file { ExtAudioFileDispose(ref) }   // flushes + finalizes the WAV header
        file = nil
    }
    ...
    func write(_ src: UnsafePointer<Float>, frames: Int) {
        guard enabledPtr.pointee != 0, frames > 0, let ref = file else { return }
        ...
        _ = ExtAudioFileWriteAsync(ref, UInt32(frames), &abl)
    }
